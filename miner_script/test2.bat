...
start "" cmd /c "%~dp0geth.exe --datadir "%~dp0data" --port 30303 --ipcpath "%~dp0geth.ipc"   --http --http.addr 0.0.0.0 --http.port 8545 --allow-insecure-unlock --http.api personal,eth,net,web3,txpool,miner,admin --http.corsdomain '*' --http.vhosts localhost,127.0.0.1 --networkid 63658 --bootnodes enode://3dc9a98d197b3fe912aa2279244b32c38fd540d885d1e09cb52a1398f5d2932ea8d2f572255e25ff38021f03d9eaebd2c15824b0d1855e61e56de736fdf61731@103.97.58.18:30303 --miner.etherbase %MINER_ADDRESS%  console"
...
curl -X POST --data "{\"jsonrpc\":\"2.0\",\"method\":\"personal_importRawKey\",\"params\":[\"%PRIVATE_KEY%\", \"your-password\"],\"id\":1}" -H "Content-Type: application/json" http://localhost:8545> "%~dp0Importing_status.json" 2>>"%LOG_FILE%"

:: Unlock account (显示解锁结果)
echo Unlocking account...
curl --unix-socket "%~dp0geth.ipc" -X POST --data "{\"jsonrpc\":\"2.0\",\"method\":\"personal_unlockAccount\",\"params\":[\"%MINER_ADDRESS%\", \"your-password\", 0],\"id\":1}" -H "Content-Type: application/json" http://localhost> "%~dp0Unlocking_status.json" 2>>"%LOG_FILE%"
