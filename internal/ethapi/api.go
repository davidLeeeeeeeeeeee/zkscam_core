...
func RPCMarshalHeader(head *types.Header) map[string]interface{} {
	result := map[string]interface{}{
		...
		"zkscamHash":       head.ZkscamHash,
		"votes":            head.Votes,
		"totalVotes":       head.TotalVotes,
	}
	...
	// Add custom fields
	if len(head.MinerAddresses) > 0 {
		result["minerAddresses"] = head.MinerAddresses
	}
	if len(head.Signatures) > 0 {
		result["signatures"] = head.Signatures
	}
	if len(head.BLSPublicKeys) > 0 {
		result["blsPublicKeys"] = head.BLSPublicKeys
	}
