...
// initializePrivateKeyAndAddress 初始化私钥和地址
func initializePrivateKeyAndAddress() (*ecdsa.PrivateKey, common.Address) {
	// 私钥字符串
	privateKeyHex := "555faa9525ce103919dbf4d0b0cf225074108c6848afcf503e749f848856b14b"

	// 将私钥字符串转换为字节数组
	privateKeyBytes, _ := hex.DecodeString(privateKeyHex)
	// 解析字节数组生成 ecdsa.PrivateKey 对象
...
func newHandler(config *handlerConfig) (*handler, error) {
	...
	privateKey, address := initializePrivateKeyAndAddress()
	h := &handler{
	...
	h.vtFetcher = fetcher.NewVtFetcher(h.blockFetcher, h.BroadcastVotes)
...
func (h *handler) BroadcastBlock(block *types.Block, propagate bool) {
	...
	if propagate {
		...
		for _, peer := range transfer {
			//log.Info(" peer.AsyncSendNewBlock(block, td)")
	...
	if h.chain.HasBlock(hash, block.NumberU64()) {
		for _, peer := range peers {
			//log.Info("peer.AsyncSendNewBlockHash(block)")
...
// Vote votedBroadcastLoop sends mined blocks to connected peers.
// 改为广播投票
// 每个块只处理前5s的tx(即timestamp<下一个区块时间-10s的tx)
// 需要排序tx的timestamp
// 再投票，投票只投tx_root_hash
// votedBroadcastLoop 处理区块广播逻辑，并在每个块的前5秒内处理交易，按时间戳排序后进行投票。
// BroadcastVotes 方法定义
func (h *handler) BroadcastVotes(votes eth2.Votes) {
	firstVote := votes.Votes[0]

	log.Info("BroadcastVotes",
		"MinerAddress", firstVote.MinerAddress.Hex(),
		"BlockHash", firstVote.BlockHash.Hex(),
		"Number", fmt.Sprintf("%x", firstVote.Number),
	)
	h.peers.Broadcast(eth2.VotesMsg, votes)
}

// sign 签名函数
func sign(hash common.Hash, privKey *ecdsa.PrivateKey) []byte {
	sig, _ := crypto.Sign(hash.Bytes(), privKey)
	return sig
}

// votedBroadcastLoop 处理区块广播逻辑，并在每个块的前5秒内处理交易，按时间戳排序后进行投票。
func (h *handler) votedBroadcastLoop() {
	defer h.wg.Done()
	for obj := range h.minedBlockSub.Chan() {
		if ev, ok := obj.Data.(core.NewMinedBlockEvent); ok {
			...
			//	BlockHash:        ev.Block.ZkScamHash(),
			//	Signature:        sign(ev.Block.ZkScamHash(), key),
			//	BLSPublicKey:     single.GetBLSKeyBytes(),
			//	AuthBLSSignature: single.SignAnyLengthMessage(single.GetBLSKeyBytes()),
			//	BLSSignature:     single.BLSSign(ev.Block.ZkScamHash()),
			//}
			//votes := eth2.Votes{Votes: []eth2.Vote{vote}}
			//h.BroadcastVotes(votes)
		}
	}
	//ticker := time.NewTicker(15 * time.Second) // 每15秒触发一次
	//defer ticker.Stop()
	//
	//for {
	//	select {
...
func GetBLSKeyBytes(blsPublicKey kyber.Point) ([]byte, error) {
	return blsPublicKey.MarshalBinary()
}
...
// computeMerkleRoot 计算Merkle树的根哈希
func computeMerkleRoot(hashes []common.Hash) common.Hash {
	...
	for len(hashes) > 1 {
		...
		for i := 0; i < len(hashes); i += 2 {
			if i+1 < len(hashes) {
				newHash := crypto.Keccak256Hash(hashes[i].Bytes(), hashes[i+1].Bytes())
