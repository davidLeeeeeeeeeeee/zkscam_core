...
func (f *BlockFetcher) Notify(peer string, hash common.Hash, number uint64, time time.Time,
	...
	log.Info("func (f *BlockFetcher) Notify(peer string")
	block := &blockAnnounce{
		hash:        hash,
		number:      number,
		time:        time,
...
// events.
// 接收新区块的逻辑在 case notification := <-f.notify: 处，流程如下：
//
// 接收到区块通知：
//
// notification 是收到的区块通知，这里的通知记录了区块的哈希、发送该通知的节点 (origin)、以及区块的高度 (number)。
// DOS 检测：
//
// 每个节点能发送的未处理区块通知有上限（hashLimit）。如果一个节点超过了这个限制，系统会记录并丢弃其发送的区块。
// 有效性检查：
//
// 检查区块号 (notification.number)，确保该区块的高度合适，且不超出预设的区块间距（maxQueueDist 和 maxUncleDist）。
// 如果区块号不在合理的范围内，系统会丢弃该区块。
// 区块未在下载中：
//
// 检查区块是否已经在下载中。如果已经在 f.fetching 或 f.completing 队列中，则跳过处理。
// 加入区块通知列表：
//
// 如果区块通知通过了上述检查，系统会将该通知加入 f.announced，这是一个用来记录接收到的区块通知的队列。
// 如果这个哈希是第一次被宣布，则会调用 f.announceChangeHook。
// 重新调度区块获取：
//
// 如果这是唯一一个被宣布的区块，会调用 f.rescheduleFetch(fetchTimer) 来重新调度区块的获取。
// 之后：
// 如果这个区块仍未下载，超时或者条件满足，会触发 fetchTimer.C 或 completeTimer.C，系统会向相应的节点请求完整区块头或区块体并完成下载。
	...
	for {
		...
		for !f.queue.Empty() {
			...
			if (number+maxUncleDist < height) || (f.light && f.getHeader(hash) != nil) || (!f.light && f.getBlock(hash) != nil) {
				log.Info(" 1.......f.forgetBlock(hash)")
				f.forgetBlock(hash)
				continue
			}
			if f.light {
				log.Info("f.importHeaders(op.origin, op.header)")
				f.importHeaders(op.origin, op.header)
			} else {
				log.Info("f.importBlocks(op.origin, op.block)")
		...
		case notification := <-f.notify:
			...
			log.Info("case notification := <-f.notify:")
			...
			if f.announceChangeHook != nil && len(f.announced[notification.hash]) == 1 {
				log.Info("f.announceChangeHook(notification.hash, true)")
				f.announceChangeHook(notification.hash, true)
			}
			if len(f.announced) == 1 {
				log.Info("f.rescheduleFetch(fetchTimer)")
				f.rescheduleFetch(fetchTimer)
			}

		case op := <-f.inject:
			log.Info("case op := <-f.inject:")
		...
		case <-fetchTimer.C:
			...
			for hash, announces := range f.announced {
				...
				if time.Since(announces[0].time) > timeout {
					...
					log.Info("fetchTimer.C f.forgetHash(hash)")
					f.forgetHash(hash)
...
func (f *BlockFetcher) enqueue(peer string, header *types.Header, block *types.Block) {
	log.Info("(f *BlockFetcher) enqueue(")
	var (
		hash   common.Hash
		number uint64
	)
	...
	if _, ok := f.queued[hash]; !ok {
		...
		log.Info("Queued delivered header or block", "peer", peer, "number", number, "hash", hash, "queued", f.queue.Size())
	}
}
...
func (f *BlockFetcher) importBlocks(peer string, block *types.Block) {
	...
	go func() {
		...
		case consensus.ErrFutureBlock:
			log.Error("case consensus.ErrFutureBlock:")
		...
		// Run the actual import and log any issues
		log.Info("if _, err := f.insertChain(types.Blocks{block}); err != nil {")
...
func (f *BlockFetcher) forgetHash(hash common.Hash) {
	...
	if announceMap, ok := f.announced[hash]; ok {
		...
		log.Info("forgetHash delete(f.announced, hash)")
