...
func NewPeer(version uint, p *p2p.Peer, rw p2p.MsgReadWriter, txpool TxPool) *Peer {
	peer := &Peer{
		...
		TotalVotes:      new(big.Int), // 初始化 totalVotes
	}
...
//	}
func (p *Peer) Head() (hash common.Hash, totalVotes *big.Int) {
	p.lock.RLock()
	defer p.lock.RUnlock()

	copy(hash[:], p.head[:])
	return hash, new(big.Int).Set(p.TotalVotes)
}
...
// SetHeader updates the head hash, total difficulty, and total votes of the peer.
func (p *Peer) SetHead(hash common.Hash, td *big.Int, totalVotes *big.Int) {
	p.lock.Lock()
	defer p.lock.Unlock()

	copy(p.head[:], hash[:])
	p.td.Set(td)
	p.TotalVotes.Set(totalVotes)
}
