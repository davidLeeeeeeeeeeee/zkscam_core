...
// peerWithHighestVotes retrieves the known peer with the currently highest total votes.
func (ps *peerSet) peerWithHighestVotes() *eth.Peer {
	ps.lock.RLock()
	...
	for _, p := range ps.peers {
		if _, TotalVotes := p.Head(); bestPeer == nil || TotalVotes.Cmp(bestVotes) > 0 {
			bestPeer, bestVotes = p.Peer, TotalVotes
		}
	}
	return bestPeer
}
...
func (ps *peerSet) Broadcast(code uint64, payload interface{}) {
	ps.lock.RLock()
	defer ps.lock.RUnlock()

	for _, p := range ps.peers {
		err := p2p.Send(p.Rw, code, payload)
		if err != nil {
