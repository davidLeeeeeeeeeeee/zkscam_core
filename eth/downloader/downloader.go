...
func (d *Downloader) LegacySync(id string, head common.Hash, totalVotes *big.Int, mode SyncMode) error {
	err := d.synchronise(id, head, totalVotes, mode, false, nil)
...
func (d *Downloader) synchronise(id string, hash common.Hash, totalVotes *big.Int, mode SyncMode, beaconMode bool, beaconPing chan struct{}) error {
	log.Info("func (d *Downloader) synchronise(")
	...
	return d.syncWithPeer(p, hash, totalVotes, beaconMode)
}
...
func (d *Downloader) syncWithPeer(p *peerConnection, hash common.Hash, totalVotes *big.Int, beaconMode bool) (err error) {
	...
	if !beaconMode {
		...
		log.Info("origin, err = d.findAncestor(p, latest)")
	...
	fetchers := []func() error{
		...
		func() error { return d.processHeaders(origin+1, totalVotes, beaconMode) },
	}
	if mode == SnapSync {
		d.pivotLock.Lock()
		d.pivotHeader = pivot
		...
		fetchers = append(fetchers, func() error { return d.processFullSyncContent(totalVotes, beaconMode) })
	}
...
func (d *Downloader) processHeaders(origin uint64, totalVotes *big.Int, beaconMode bool) error {
	var (
		mode       = d.getMode()
		...
		case task := <-d.headerProcCh:
			...
			if task == nil || len(task.headers) == 0 {
				...
				if !beaconMode {
					...
					if mode != LightSync {
						...
						if !gotHeaders && totalVotes.Cmp(d.blockchain.GetTd(head.Hash(), head.Number.Uint64())) > 0 {
					...
					if mode == SnapSync || mode == LightSync {
						...
						if totalVotes.Cmp(d.lightchain.GetTd(head.Hash(), head.Number.Uint64())) > 0 {
							return errStallingPeer
						}
					}
				}
			...
			for len(headers) > 0 {
				...
				if mode == SnapSync || mode == LightSync {
					...
					if len(rejected) != 0 {
						log.Info("Legacy sync reached merge threshold", "number", rejected[0].Number, "hash", rejected[0].Hash(), "td", td, "totalVotes", totalVotes)
...
func (d *Downloader) processFullSyncContent(totalVotes *big.Int, beaconMode bool) error {
	for {
		...
		if !beaconMode && totalVotes != nil {
			...
			if td == nil {
				// This should never really happen, but handle gracefully for now
				log.Error("Failed to retrieve parent block TotalVotes", "number", results[0].Header.Number.Uint64()-1, "hash", results[0].Header.ParentHash)
				return fmt.Errorf("%w: parent TD missing", errInvalidChain)
			}
			for i, result := range results {
				td = new(big.Int).Add(td, result.Header.TotalVotes)
				if td.Cmp(totalVotes) >= 0 {
					// Terminal total difficulty reached, allow the last block in
					if new(big.Int).Sub(td, result.Header.TotalVotes).Cmp(totalVotes) < 0 {
						results, rejected = results[:i+1], results[i+1:]
						if len(rejected) > 0 {
							// Make a nicer user log as to the first TD truly rejected
							td = new(big.Int).Add(td, rejected[0].Header.TotalVotes)
						}
					} else {
		...
		if len(rejected) != 0 {
			log.Info("Legacy sync reached merge threshold", "number", rejected[0].Header.Number, "hash", rejected[0].Header.Hash(), "td", td, "totalVotes", totalVotes)
			return ErrMergeTransition
		}
	}
}
