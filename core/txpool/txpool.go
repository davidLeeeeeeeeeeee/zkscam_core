...
func (p *TxPool) Add(txs []*types.Transaction, local bool, sync bool) []error {
	// Split the input transactions between the subpools.
	txsets := make([][]*types.Transaction, len(p.subpools))
	splits := make([]int, len(txs))
	var fixedGasPrice = big.NewInt(1000000000) // 固定的 gasPrice 为 1 Gwei

	for i, tx := range txs {
		// 检查交易的 gasPrice 是否符合固定的 gasPrice
		if tx.GasPrice().Cmp(fixedGasPrice) < 0 {
			// 如果 gasPrice 不匹配，返回错误并跳过该交易
			log.Warn("Transaction rejected due to incorrect gas price", "txHash", tx.Hash(), "gasPrice", tx.GasPrice())
			splits[i] = -1
			continue
		}
