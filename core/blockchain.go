...
func (bc *BlockChain) insertChain(total_chain types.Blocks, setHead bool) (int, error) {
	...
	SenderCacher.RecoverFromBlocks(types.MakeSigner(bc.chainConfig, total_chain[0].Number(), total_chain[0].Time()), total_chain)
	...
	var total_index = 0
	var err error = nil
	for _, block := range total_chain {
		// 将每个 block 封装成一个单独的 types.Blocks 列表
		chain := types.Blocks{block}
...
// reorg 接受两个区块，一个旧链和一个新链，并将重新构建这些区块，
// 将它们插入到新的规范链中，同时累积潜在缺失的交易并发布有关它们的事件。
// 注意，新的链头区块不会在这里处理，调用者需要在外部处理它。
func (bc *BlockChain) reorg(oldHead *types.Header, newHead *types.Block) error {
	...
	if oldBlock == nil {
		return errors.New("current head block missing") // 当前链头区块缺失
	}
	newBlock := newHead

	// 将较长的链减少到与较短链相同的高度
	if oldBlock.NumberU64() > newBlock.NumberU64() {
		// 旧链更长，将所有交易和日志收集为待删除的
		...
		// 新链更长，将所有区块暂时存储以便后续插入
		for ; newBlock != nil && newBlock.NumberU64() != oldBlock.NumberU64(); newBlock = bc.GetBlock(newBlock.ParentHash(), newBlock.NumberU64()-1) {
			newChain = append(newChain, newBlock)
		}
	}
	if oldBlock == nil {
		return errInvalidOldChain // 旧链无效
	}
	if newBlock == nil {
		return errInvalidNewChain // 新链无效
	}
	// 两边的重组链都处于相同的区块号，继续回溯，直到找到共同祖先
	for {
		// 如果找到了共同祖先，退出循环
	...
	if len(oldChain) > 0 && len(newChain) > 0 {
		...
		logFn(msg, "区块号", commonBlock.Number(), "哈希", commonBlock.Hash(),
			"删除区块数", len(oldChain), "从此区块删除", oldChain[0].Hash(), "添加区块数", len(newChain), "从此区块添加", newChain[0].Hash())
		blockReorgAddMeter.Mark(int64(len(newChain)))
		blockReorgDropMeter.Mark(int64(len(oldChain)))
		blockReorgMeter.Mark(1)
	} else if len(newChain) > 0 {
		// 在合并后阶段的特殊情况，当前链头是新链头的祖先，但这两个区块并不连续
		log.Info("扩展链", "添加区块数", len(newChain), "区块号", newChain[0].Number(), "哈希", newChain[0].Hash())
		blockReorgAddMeter.Mark(int64(len(newChain)))
	} else {
		// len(newChain) == 0 && len(oldChain) > 0
		// 将规范链回滚到较低的点。
		log.Error("不可能的重组，请提交问题报告", "旧区块号", oldBlock.Number(), "旧哈希", oldBlock.Hash(), "旧链区块数", len(oldChain), "新区块号", newBlock.Number(), "新哈希", newBlock.Hash(), "新链区块数", len(newChain))
	}
	// 重置交易查找缓存，以便清除过时的查找项。
	// 这在写入任何新链数据之前完成，以避免出现规范链更改但缓存仍然保留旧查找项的怪异情况。
	bc.txLookupCache.Purge()
	...
	// 立即删除无用的索引，其中包括非规范交易索引，规范链头之上的索引。
	var (
	...
	// 删除所有不属于新规范链的哈希标记。
	// 因为重组函数不处理新链头，所以所有大于等于新链头的哈希标记应该被删除。
	...
	// 为旧规范链中的日志和区块发送事件，并为新规范链中的'重生'日志发送事件。日志的数量可能非常多，
	// 因此事件将以大约512条日志为一批次发送。
...
func (bc *BlockChain) SetCanonical(head *types.Block) (common.Hash, error) {
	...
	if head.ParentHash() != bc.CurrentBlock().Hash() {
		log.Info("if head.ParentHash() != bc.CurrentBlock().Hash() {")
